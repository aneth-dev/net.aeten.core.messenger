...
public class DefaultServiceLoader implements ServiceLoader {
	...
	protected Collection<Class<?>>					unassociatedProviders	= new ArrayList<Class<?>>();
	...
	public synchronized void registerService(Class<?> service) {
		Logger.log(this, LogLevel.TRACE, "Register service " + service.getName());
		List<Class<?>> providers = new ArrayList<Class<?>>();
		for (Class<?> provider : unassociatedProviders) {
			if (service.isAssignableFrom(provider)) {
				Logger.log(this, LogLevel.TRACE, "Register provider " + provider.getName() + " for service " + service.getName());
				providers.add(provider);
			}
		}
		for (Class<?> provider : providers) {
			unassociatedProviders.remove(provider);
		}
		services.put(service, providers);
	}

	public synchronized <T> void registerProvider(Class<T> service, T provider) {
		Logger.log(this, LogLevel.TRACE, "Register provider instance " + provider + " for service " + service.getName());
	...
	protected synchronized <T> void registerProvider(Class<? extends T> provider) {
		for (Map.Entry<Class<?>, Collection<Class<?>>> serviceEntry : services.entrySet()) {
			if (serviceEntry.getKey().isAssignableFrom(provider)) {
				Logger.log(this, LogLevel.TRACE, "Register provider " + provider.getName() + " for service " + serviceEntry.getKey().getName());
				serviceEntry.getValue().add(provider);
				return;
			}
		}
		unassociatedProviders.add(provider);
	}
						...
						if (Configurable.class.isAssignableFrom(clazz)) {
							...
							while (directories.hasMoreElements()) {
								...
								for (URL fileUrl : files) {
									...
									provider.configure(Yaml2Args.convert(new InputStreamReader(fileUrl.openStream(), "UTF-8"), (Parser<MarkupNode>) Service.getProvider(Parser.class, PmlParser.class.getName())));
