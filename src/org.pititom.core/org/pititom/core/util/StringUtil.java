...
public class StringUtil {
	...
	public static String[] splitWithQuote(String input, char quote) throws IllegalArgumentException {
		return splitWithQuote(input, quote, quote);
	}

	public static String[] splitWithQuote(String input, char openQuote, char closeQuote) throws IllegalArgumentException {
		if (input == null) {
			return new String[] {};
		}
		...
		for (int i = 0; i < input.length(); i++) {
			...
			if (begin > i)
				addAll(output, new String(input.getBytes(), i, begin - i).trim().split("\\s+"));
			if (begin == -1) {
			...
			if (end == -1)
				throw new IllegalArgumentException("Unclosed quote at char " + begin + ". Input=" + input);
			i = end;
		}
		return output.toArray(new String[output.size()]);
	}

	private static int findNextQuote(String arguments, int begin, char quote) {
	...
	private static void addAll(LinkedList<String> list, String[] elements) {
		for (String element : elements)
			list.add(element);
	}

	public static void main(String[] arguments) {
		doAssertion(splitWithQuote("-a toto -b titi"), new String[] { "-a", "toto", "-b", "titi" });
		doAssertion(splitWithQuote("-a toto -b titi -c \"tutu titi\\\"\" -d tata"), new String[] { "-a", "toto", "-b", "titi", "-c", "tutu titi\"", "-d", "tata", });
		doAssertion(splitWithQuote("-a toto -b \"-c \\\"titi\\\"\""), new String[] { "-a", "toto", "-b", "-c \"titi\"" });
